...
import { Upload, User, Mail, MapPin, Phone, Lock } from "lucide-react";
      ...
      {error || helperText ? (
        <div className="mt-1">
          {error ? (
            ...
            <p className="text-xs font-medium text-text-muted">{helperText}</p>
          )}
        </div>
      ) : null}
    </div>
...
const AdminAddUser = () => {
  ...
  const inputClass = (hasError) =>
    [
      "w-full rounded-xl border bg-slate-50 px-4 py-2.5 text-sm font-medium text-text-primary outline-none transition",
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmation = await Swal.fire({
      ...
      text: "Please confirm the details before creating the user.",
      icon: "question",
      ...
      customClass: {
        ...
        confirmButton:
          "bg-brand hover:bg-brand-dark text-white rounded-xl px-6 py-2.5 font-bold text-sm transition-colors mx-2",
        cancelButton:
          "bg-background hover:bg-gray-100 text-text-primary rounded-xl px-6 py-2.5 font-bold text-sm transition-colors mx-2"
      }
    });
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="flex flex-col items-center gap-2.5 mb-8">
              <div className="relative group h-24 w-24 rounded-full overflow-hidden border-4 border-slate-100 bg-slate-50 shadow-inner">
                ...
                <label
                  htmlFor="file"
                  className="absolute inset-0 bg-brand/60 flex flex-col items-center justify-center text-[10px] font-bold text-white opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer uppercase tracking-wider"
                >
              ...
              <div className="text-center">
                <span className="text-[11px] font-bold text-text-muted uppercase tracking-wider block">
                  {form.file ? (
                    <span className="text-brand truncate max-w-[220px] block">
                      {form.file.name}
                    </span>
                  ) : (
                    "Upload Photo"
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end">
              <button
                ...
                disabled={isLoading}
                className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-text-primary transition hover disabled:cursor-not-allowed disabled:opacity-60"
              >
                Cancel
              </button>
