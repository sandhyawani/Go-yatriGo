...
const TravelBuddyDetails = () => {
  ...
  const fetchTripDetails = async () => {
    try {
      const res = await axios.get(`/social/buddy/${id}`, { withCredentials: true });
  ...
  const handleSendRequest = async (e) => {
    ...
    try {
      const res = await axios.post(`/social/buddy/join-request/${id}`, { message: requestMessage }, { withCredentials: true });
      setRequestMessage("");
  ...
  const handleManageRequest = async (requestId, status) => {
    try {
      await axios.post(`/social/buddy/manage-request/${id}`, { requestId, status }, { withCredentials: true });
  ...
  const handleManageMember = async () => {
    ...
    try {
      ...
      if (type === 'warn') {
        await axios.post(`/social/buddy-trips/${id}/warn/${memberId}`, { message: warningMsg }, { withCredentials: true });
        showToast.success("Warning sent");
      } else if (type === 'ban') {
        await axios.post(`/social/buddy-trips/${id}/ban/${memberId}`, {}, { withCredentials: true });
        showToast.success("User banned");
      } else if (type === 'remove') {
        await axios.delete(`/social/buddy-trips/${id}/member/${memberId}`, { withCredentials: true });
        showToast.success("Member removed");
      } else if (type === 'promote') {
        await axios.post(`/social/buddy-trips/${id}/promote/${memberId}`, {}, { withCredentials: true });
  ...
  const handleLeaveTrip = async () => {
    try {
      await axios.post(`/social/buddy/leave/${id}`, {}, { withCredentials: true });
  ...
  const handleCancelTrip = async () => {
    try {
      await axios.patch(`/social/buddy/${id}/cancel`, { cancellationReason }, { withCredentials: true });
      showToast.success("Travel group cancelled");
      setTrip(prev => ({ ...prev, status: "cancelled", lifecycleStatus: "cancelled" }));
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await axios.post(`/social/buddy/like/${id}`, {}, { withCredentials: true });
      setTrip(prev => ({
        ...prev,
        likes: res.data.isLiked 
          ? [...(prev.likes || []), user._id] 
          : (prev.likes || []).filter(lid => lid?.toString() !== user._id?.toString())
      }));
  ...
  const isHost = user && (trip.userId?._id || trip.userId)?.toString() === currentUserId;
  const myMemberObj = trip?.members?.find(m => (m?.user?._id || m?.user)?.toString() === currentUserId);
  const myRole = isHost ? "host" : (myMemberObj?.role || "member");
  const isCompanion = user && (trip.companions?.some(c => (c?._id || c)?.toString() === currentUserId) || trip.members?.some(m => (m?._id || m)?.toString() === currentUserId));
  const userRequest = user && trip.joinRequests?.find(r => (r?.userId?._id || r?.userId)?.toString() === currentUserId);
  ...
  const hasFelt = trip.likes?.some(likeId => likeId?.toString() === currentUserId);
    ...
    <div className="bg-[#FAFAFA] text-[#111827] pt-4 sm:pt-5 pb-20 px-4 sm:px-6 lg:px-8 font-sans antialiased">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleLike} aria-label={hasFelt ? "Remove Felt reaction" : "Felt This"} className={`inline-flex items-center gap-1.5 px-3 py-1.5 border rounded-xl font-medium text-sm transition-all ${ hasFelt ? "bg-[#FAFAFA] border-[#E5E7EB] text-[#111827] hover:bg-slate-50" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50" }`}
            >
            ...
            <div className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] font-semibold text-[#6C4DF6] capitalize">{trip.userId?.type || "Traveler"}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2 text-center pt-2">
                <div className="bg-[#FAFAFA] p-2.5 rounded-xl border border-slate-100">
                  <span className="text-[8px] font-medium text-slate-400 block mb-0.5">Rating</span>
                ...
                <div className="bg-[#FAFAFA] p-2.5 rounded-xl border border-slate-100">
                  <span className="text-[8px] font-black text-slate-400 block mb-0.5">Trips</span>
                  <span className="text-xs font-extrabold text-slate-800">{trip.userId?.completedTrips || 0} hosted</span>
                </div>
              </div>
              
              <div className="bg-[#FAFAFA] p-2.5 rounded-xl border border-slate-100 text-center mt-2">
                <span className="text-[8px] font-black text-slate-400 block mb-0.5">Response Rate</span>
              ...
              {trip.userId?.interests && trip.userId.interests.length > 0 && (
                <div className="pt-2">
                  <span className="text-[8px] font-medium text-slate-400 block mb-1.5">Interests</span>
                  <div className="flex flex-wrap gap-1">
                    {trip.userId.interests?.map(interest => (
                      <span key={interest} className="bg-slate-50 border border-slate-200 px-2 py-0.5 rounded text-[8px] font-bold text-slate-500 shadow-sm">
                        {interest}
                      </span>
                    ))}
                  </div>
            ...
            {!isHost && (
              <div className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm space-y-3">
                ...
                {isCompanion || isApproved ? (
                  ...
                  <button
                    disabled
                    className="w-full py-2.5 bg-emerald-500 text-white rounded-xl font-black text-[10px] shadow-md flex items-center justify-center gap-1.5 opacity-100 cursor-default"
                  >
              ...
              ) : trip.lifecycleStatus === 'completed' || trip.lifecycleStatus === 'cancelled' ? (
                <div className="bg-slate-50 border border-slate-100 text-slate-500 p-3 rounded-xl text-xs font-semibold flex items-start gap-2">
                  ...
                  <span>This group is no longer active and cannot be joined.</span>
                </div>
              ) : trip.status === 'full' ? (
                ...
                <form onSubmit={handleSendRequest} className="space-y-3">
                  ...
                  >
                    <UserPlus className="w-4 h-4" /> {submittingRequest ? "Submitting..." : "Request to Join"}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleSendRequest} className="space-y-3">
                  <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                    This is a public group. You can join instantly without approval.
                  </p>
                  <button
                    type="submit"
                    disabled={submittingRequest}
                    className="w-full py-2.5 bg-[#7F77DD] hover:bg-[#6C4DF6] text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center gap-1.5 disabled:opacity-60"
                  >
                    <UserPlus className="w-4 h-4" /> {submittingRequest ? "Joining..." : "Join Group"}
                  </button>
                </form>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm space-y-3">
                ...
                <div className="space-y-3 max-h-[250px] overflow-y-auto custom-scrollbar">
                  {pendingRequests.length === 0 ? (
                    <p className="text-xs text-slate-400 font-semibold py-3 text-center">No pending requests.</p>
                  ) : (
                    pendingRequests.map(req => (
                      <div key={req._id} className="bg-[#FAFAFA] p-3 rounded-xl border border-slate-100 space-y-2">
                        <div className="flex items-center gap-2">
                          <img
                          ...
                          />
                          <div className="min-w-0">
                            <span className="text-xs font-black text-[#111827] block truncate">{req.userId?.name || "Traveler"}</span>
                            <span className="text-[9px] text-slate-500 font-semibold">Rating {req.userId?.rating || "4.6"}</span>
                          </div>
                        </div>
                        {req.message && (
            ...
            <div className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm space-y-3">
              ...
              <div className="space-y-3 pb-20">
                {trip.members?.map((memberObj) => {
                  ...
                  const canManage = (myRole === 'host' || myRole === 'cohost') && !isMe && !isTargetHost;
                  
                  return (
                    <div key={mId} className="flex items-center justify-between bg-[#FAFAFA] p-3 rounded-xl border border-slate-100 relative group">
                      <div className="flex items-center gap-3">
                        <img
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1">
                            ...
                            >{mUser.name || "User"}</h4>
                            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${ memberObj.role === 'host' ? 'bg-[#6C4DF6]/10 text-[#6C4DF6]' : memberObj.role === 'cohost' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500' }`}>
                              {memberObj.role}
                            </span>
                          </div>
                          <span className="text-[9px] text-slate-500 font-semibold">Rating {mUser.rating || "4.5"}</span>
                        </div>
                      </div>
                        ...
                        <div className="relative">
                          ...
                          {openDropdownId === mId && (
                            <div className="absolute right-0 top-full mt-1 w-40 bg-white/90 backdrop-blur-xl border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
                              <button onClick={() => { setManageAction({ type: 'warn', memberId: mId, memberName: mUser.name }); setOpenDropdownId(null); }} className="w-full text-left px-4 py-2 text-xs font-semibold text-amber-600 hover:bg-slate-50">Send Warning</button>
                              {myRole === 'host' && (
                                <button onClick={() => { setManageAction({ type: 'promote', memberId: mId, memberName: mUser.name }); setOpenDropdownId(null); }} className="w-full text-left px-4 py-2 text-xs font-semibold text-[#6C4DF6] hover:bg-slate-50">
                                  {memberObj.role === 'cohost' ? 'Demote to Member' : 'Make Co-host'}
                                </button>
                              )}
                              <button onClick={() => { setManageAction({ type: 'remove', memberId: mId, memberName: mUser.name }); setOpenDropdownId(null); }} className="w-full text-left px-4 py-2 text-xs font-semibold text-rose-500 hover:bg-slate-50 border-t border-slate-100">Remove Member</button>
                              {myRole === 'host' && (
                                <button onClick={() => { setManageAction({ type: 'ban', memberId: mId, memberName: mUser.name }); setOpenDropdownId(null); }} className="w-full text-left px-4 py-2 text-xs font-black text-rose-600 hover:bg-rose-50">Ban User</button>
                              )}
                            </div>
                          )}
                        </div>
      ...
      {manageAction && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            <h3 className="text-xl font-black text-[#111827] mb-2 flex items-center gap-2">
              <AlertTriangle className={`w-5 h-5 ${manageAction.type === 'remove' || manageAction.type === 'ban' ? 'text-rose-500' : 'text-amber-500'}`} />
              {manageAction.type === 'warn' ? 'Send Warning' : 
               manageAction.type === 'ban' ? 'Ban User' : 
               manageAction.type === 'remove' ? 'Remove Member' : 'Change Role'}
            </h3>
            
            <p className="text-xs font-semibold text-slate-500 mb-4">
              {manageAction.type === 'warn' && `Send a warning to ${manageAction.memberName}.`}
              {manageAction.type === 'remove' && `Are you sure you want to remove ${manageAction.memberName}? They will lose access to the group chat.`}
              {manageAction.type === 'ban' && `Are you sure you want to permanently ban ${manageAction.memberName}? They will not be able to rejoin.`}
              {manageAction.type === 'promote' && `Change the role of ${manageAction.memberName}.`}
            </p>
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button
                onClick={() => { setManageAction(null); setWarningMsg(""); }}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <p className="text-xs font-semibold text-slate-500 mb-6">
              Are you sure you want to leave this travel group? You will lose access to group chat and trip updates.
            </p>
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            <h3 className="text-xl font-black text-[#111827] mb-2">Cancel this trip?</h3>
            <p className="text-xs font-semibold text-slate-500 mb-4">
