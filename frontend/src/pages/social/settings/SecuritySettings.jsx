...
const parseUserAgent = (uaString) => {
  ...
  if (ua.includes("safari")) browser = "Safari";else
  if (ua.includes("opera") || ua.includes("opr/")) browser = "Opera";

  let os = "Unknown OS";
  if (ua.includes("win")) os = "Windows";else
  if (ua.includes("mac")) os = "macOS";else
...
<div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
  ...
  style={{
    ...
    <div>
      ...
      {subtitle ?
    <p style={{ fontSize: "12px", color: "#64748B", margin: 0, marginTop: "2px", lineHeight: "1.4" }}>
          {subtitle}
        </p> :
    null}
    </div>
...
const SecuritySettings = () => {
  ...
  const queryTab = useMemo(() => {
    ...
    if (tabParam && ["all", "password", "2fa", "sessions"].includes(tabParam)) {
      return tabParam;
    }
    const hash = location.hash.replace("#", "");
    if (hash && ["all", "password", "2fa", "sessions"].includes(hash)) {
      return hash;
    }
    return "all";
  ...
  const handleTabClick = (tabId) => {
    setActiveTab(tabId);
    const basePath = isAdminMode ? "/admin/settings/security" : "/settings/security";
    if (tabId === "all") {
      navigate(basePath);
    } else {
