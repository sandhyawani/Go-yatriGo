    ...
    <div className=" bg-slate-50 pt-14 md:pt-0 pb-20">
      <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-text-muted font-medium mt-1">Manage accounts you have blocked</p>
        </div>

        <div className="bg-surface rounded-[var(--radius-card)] p-2 shadow-sm border border-slate-100">
          {loading ?
          ...
          <div className="text-center py-16">
              ...
              <p className="text-text-muted font-medium">No blocked users.</p>
            </div> :
            ...
            <div key={user._id} className="p-4 flex items-center justify-between">
                ...
                className="w-12 h-12 rounded-full object-cover border border-slate-200" />
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-text-muted">@{user.username || user._id.toString().slice(-6)}</p>
                    </div>
                  </div>
                  <button onClick={() => handleUnblock(user._id)} className="px-4 py-2 bg-background hover text-text-primary text-xs font-bold rounded-xl transition-colors flex items-center gap-2">
