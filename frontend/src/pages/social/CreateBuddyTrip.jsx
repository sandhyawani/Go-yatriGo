...
const CreateBuddyTrip = () => {
  ...
  const predefinedTags = ["luxury", "budget", "students", "family", "photography", "spiritual", "trekking", "roadtrip", "weekend", "foodie"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { ...formData, maxCompanions: formData.maxMembers, coverImage: imageUrl };
      const res = await axios.post("/social/buddy", payload, { withCredentials: true });
      ...
      toast.error(err.response?.data?.message || "Failed to create group", { id: toastId });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans pb-24 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <form id="createBuddyTripForm" onSubmit={handleSubmit} className="space-y-8">
          ...
          >
            ...
            >
              {imagePreview ? (
                <>
                  <img src={imagePreview} alt="Cover" className="w-full h-full object-cover absolute inset-0" />
                ...
                <div className="flex flex-col items-center text-indigo-400">
                  ...
                  <span className="text-sm font-bold text-slate-700 mb-1">Upload Trip Cover</span>
                  <span className="text-xs text-slate-400 font-medium">Recommended: 16:9, Max 5MB</span>
                </div>
              )}
            ...
            <div className="md:col-span-2 space-y-8">
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-lg font-bold text-slate-800">Trip Basics</h2>
                </div>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input
                      type="text" name="title" placeholder="e.g. Mystical Manali Weekend" value={formData.title} onChange={handleInputChange}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Starting From</label>
                      <div className="relative">
                        <MapPin className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${errors.from ? 'text-rose-400' : 'text-slate-400'}`} />
                        <input
                          type="text" name="from" placeholder="City or Landmark" value={formData.from} onChange={handleInputChange}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <MapPin className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${errors.destination ? 'text-rose-400' : 'text-indigo-400'}`} />
                        <input
                          type="text" name="destination" placeholder="Where to?" value={formData.destination} onChange={handleInputChange}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="date" name="startDate" value={formData.startDate} min={todayStr} onChange={handleInputChange}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                        <input
                          type="date" name="endDate" value={formData.endDate} min={formData.startDate || todayStr} onChange={handleInputChange}
              ...
              >
                ...
                <div className="space-y-5">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Description & Itinerary</label>
                    <textarea
                      name="description" rows="5" placeholder="What's the plan? Describe the vibe, places you'll visit, and what kind of travel buddies you're looking for..." 
                    ...
                    <div className="flex justify-between items-center mt-1.5">
                      ...
                      <span className={`text-xs font-medium ml-auto ${formData.description.length < 20 ? 'text-rose-400' : 'text-emerald-500'}`}>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Vibe Tags</label>
            ...
            <div className="space-y-8">
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-base font-bold text-slate-800">Budget Estimate</h3>
                </div>

                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-semibold text-lg">₹</span>
                  <input
                    type="number" name="budget" placeholder="0" value={formData.budget} onChange={handleInputChange} min="0"
                ...
                <p className="text-xs text-slate-500 font-medium mt-2 ml-1">Approximate cost per person</p>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-base font-bold text-slate-800">Group Rules</h3>
                </div>

                <div>
                  <div className="flex justify-between items-center mb-2 ml-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Group Size</label>
                    <span className="text-sm font-extrabold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">{formData.maxMembers}</span>
                  </div>
                  <input 
                    type="range" min="2" max="20" value={formData.maxMembers}
                ...
                <div className="pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between group cursor-pointer" onClick={() => setFormData(p => ({ ...p, isPrivate: !p.isPrivate }))}>
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg transition-colors ${formData.isPrivate ? 'bg-slate-100 text-slate-700' : 'bg-emerald-50 text-emerald-600'}`}>
                        {formData.isPrivate ? <Lock className="w-4 h-4" /> : <Globe className="w-4 h-4" />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-800">{formData.isPrivate ? "Private Group" : "Public Group"}</p>
                        <p className="text-xs font-medium text-slate-400 mt-0.5">{formData.isPrivate ? "Invite only" : "Anyone can join"}</p>
                      </div>
                    </div>
                    
                    <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.isPrivate ? 'bg-slate-300' : 'bg-emerald-500'}`}>
