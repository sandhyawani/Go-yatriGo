...
import {
  Heart, MessageSquare, Trash2, Send, Plus, X,
  MapPin, Compass, UserPlus, Sparkles,
  Bookmark, Share2, Search, Bell, Loader2, ShieldAlert,
  Music2, Play, Pause, ChevronLeft, ChevronRight, Edit2, MoreHorizontal, Clock
} from "lucide-react";
...
const notifIcon = (type) => {
  ...
  if (type === "post_comment") return <MessageSquare className="w-3.5 h-3.5 text-[#6C4DF6]" />;
...
// ─── Comment count helpers – separated for clarity ────────────────────────────────
...
const Home = () => {
  ...
  const [reportModal, setReportModal] = useState({ isOpen: false, targetId: null, targetType: "post", reportedUserId: null });
  ...
  useEffect(() => {
    ...
    const updateStoryViewers = (data) => {
      const patchStory = (st) => {
        ...
        const idx = viewers.findIndex(v => v.userId?._id === data.viewer.userId?._id);
        if (idx !== -1) viewers[idx] = data.viewer;
        else viewers.push(data.viewer);
        return { ...st, viewers };
      };
      setActiveStoryGroup(prev => prev ? { ...prev, stories: prev.stories.map(patchStory) } : prev);
      setStories(prev => prev.map(g => ({ ...g, stories: g.stories?.map(patchStory) ?? g.stories })));
    };
    ...
    const updateStoryReactions = (data) => {
      const patchStory = (st) => {
        ...
        const idx = reactions.findIndex(r => r.userId?._id === data.reaction.userId?._id);
  ...
  const fetchMemories = useCallback(async (pageNum = 1, append = false) => {
    if (pageNum === 1) { setLoadingMemories(true); setErrorMemories(false); }
    else setLoadingMore(true);
    
    try {
      const res = await axios.get(`/social/memory?page=${pageNum}&limit=10`, { withCredentials: true });
      if (res.data.success) {
        if (append) {
          setMemories(prev => {
            const existingIds = new Set(prev.map(p => p._id));
            const newPosts = res.data.memories.filter(p => !existingIds.has(p._id));
  ...
  const fetchSideData = useCallback(() => {
    ...
    Promise.allSettled([
      ...
      if (results[1].status === "fulfilled" && results[1].value.data.success) {
        const filtered = (results[1].value.data.suggestions || []).filter(s => s._id?.toString() !== myUserId);
      ...
      if (results[3].status === "fulfilled" && results[3].value.data.success) {
        const ids = new Set((results[3].value.data.posts || []).map(p => (p._id || p.postId?._id)?.toString()));
        setSavedPostIds(ids);
      }
    });
  }, [myUserId]);
  ...
  const handleOpenComments = async (postId) => {
    ...
    try {
      const res = await axios.get(`/social/memory/${postId}/comments`, { withCredentials: true });
      if (res.data.success) {
        setMemories(prev => prev.map(m => m._id === postId ? { ...m, comments: res.data.comments } : m));
      }
    } catch {
  ...
  const uploadToCloudinary = useCallback(async (file) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (res.data.success) { setPostImage(res.data.url); showToast.success("Image uploaded!"); }
  ...
  const handlePostSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "/social/memory",
        { caption: postCaption, location: postLocation, tags: tagsArray, image: postImage },
        { withCredentials: true }
      );
      if (res.data.success) {
        showToast.success("Travel memory shared! 🌍");
        setPostCaption(""); setPostLocation(""); setPostTags(""); setPostImage("");
        // Optimistically prepend to memories
        setMemories(prev => [res.data.post, ...prev]);
      }
    } catch { showToast.error("Failed to post memory"); }
  ...
  const handleLike = useCallback(async (postId) => {
    ...
    setMemories(prev => prev.map(m => {
      if (m._id === postId) {
        const hasLiked = m.likes?.some(id => (id?._id || id)?.toString() === myUserId);
        const newLikes = hasLiked
          ? (m.likes || []).filter(id => (id?._id || id)?.toString() !== myUserId)
          : [...(m.likes || []), myUserId];
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (updatedLikes) {
          setMemories(prev => prev.map(m => m._id === postId ? { ...m, likes: updatedLikes } : m));
        }
      }
    } catch (err) { 
  ...
  const handleDoubleTapLike = useCallback((postId, likes, tapPoint) => {
    const hasLiked = likes?.some(id => (id?._id || id)?.toString() === myUserId);
      ...
      if (res.data.success) {
        ...
        setMemories(prev => prev.map(m => {
          ...
          if (res.data.memory?.comments) {
            return { ...m, comments: res.data.memory.comments, commentsCount: res.data.memory.comments.length };
          }
          // Fallback: if backend returns just the comment, add it to the array
          if (res.data.comment) {
            const visibleComments = getVisibleComments(m);
            const exists = visibleComments.some(c => c._id === res.data.comment._id);
            if (exists) return m;
            const newComments = [...visibleComments, res.data.comment];
            return { ...m, comments: newComments, commentsCount: newComments.length };
          }
          return m;
        }));
      }
      ...
      if (res.data.success) {
        ...
        setMemories(prev =>
          prev.map(m => {
            ...
            const visibleComments = getVisibleComments(m);
            const newComments = visibleComments.filter(c => c._id !== commentId);
            return { ...m, comments: newComments, commentsCount: newComments.length };
          })
        );
      }
  ...
  const handleSaveToggle = useCallback(async (postId) => {
    ...
    try {
      const res = isSaved
        ? await axios.delete(`/social/memory/save/${postIdStr}`, { withCredentials: true })
        : await axios.post(`/social/memory/save/${postIdStr}`, {}, { withCredentials: true });
      if (res.data.success) {
        setSavedPostIds(prev => {
          const next = new Set(prev);
          if (!isSaved) { next.add(postIdStr); showToast.success("Post saved"); }
          else { next.delete(postIdStr); showToast.success("Removed from saved"); }
          return next;
        });
      }
    } catch { showToast.error("Failed to save post"); }
    finally { setSaveLoadingMap(prev => ({ ...prev, [postIdStr]: false })); }
  ...
  const handleStoryReply = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success) {
        showToast.success(`Reply sent to ${activeStoryGroup.userName.split(" ")[0]}! 💬`);
        setStoryReplyText("");
      }
    } catch { showToast.error("Failed to send reply"); }
    finally { setReplyingToStory(false); }
  }, [storyReplyText, activeStoryGroup, activeStoryIndex]);

  // ── Story reaction ─────────────────────────────────────────────────────────
  const handleStoryReaction = useCallback(async (emoji) => {
    ...
    try {
      ...
      if (res.data.success) { showToast.success(`${emoji} Sent!`); fetchFeedData(); }
    } catch { showToast.error("Failed to send reaction"); }
  ...
  const handleFollowToggle = useCallback(async (targetUser) => {
    ...
    const isFollowing = targetUser.followers?.some(id => id?.toString() === myUserId);
    try {
      if (isFollowing) {
        ...
        setSuggestions(prev =>
          prev.map(s =>
            s._id === targetUser._id
              ? { ...s, followers: s.followers.filter(id => id?.toString() !== myUserId) }
              : s
          )
        );
      } else {
        const res = await axios.post(`/users/${targetUser._id}/follow`, {}, { withCredentials: true });
        if (res.data.status === "requested") {
          showToast.success(`Follow request sent to ${targetUser.name}!`);
          setSuggestions(prev =>
            prev.map(s =>
              s._id === targetUser._id
                ? { ...s, followRequests: [...(s.followRequests || []), myUserId] }
                : s
            )
          );
        } else {
    ...
    finally { setFollowLoadingMap(prev => ({ ...prev, [targetUser._id]: false })); }
  ...
  const handleEditPostSubmit = async (e) => {
    ...
    try {
      ...
      if(res.data.success) {
        ...
        setMemories(prev => prev.map(p => p._id === editPostData._id ? res.data.post : p));
  ...
  const handleDeletePost = useCallback(async (postId) => {
    ...
    if (!isConfirmed) return;
    try {
      const res = await axios.delete(`/social/memory/${postId}`, { withCredentials: true });
      if (res.data.success) {
  ...
  const handleDeleteStory = useCallback(async (storyId) => {
    ...
    if (!isConfirmed) return;
    try {
      const res = await axios.delete(`/social/story/${storyId}`, { withCredentials: true });
      if (res.data.success) { showToast.success("Story deleted"); setActiveStoryGroup(null); fetchFeedData(); }
    } catch { showToast.error("Failed to delete story"); }
  }, [fetchFeedData]);
  ...
  const nextStory = useCallback(() => {
    ...
    if (activeStoryIndex < activeStoryGroup.stories.length - 1) {
      ...
      const groupIdx = stories.findIndex(g => g.userId === activeStoryGroup.userId);
  ...
  const handleSearchInput = useCallback((e) => {
    ...
    setSearchQuery(q);
    if (!q.trim()) { setSearchResults(null); setShowSearchDropdown(false); return; }
    setShowSearchDropdown(true);
    clearTimeout(searchDebounceRef.current);
    searchDebounceRef.current = setTimeout(async () => {
      setSearchLoading(true);
      try {
        const res = await axios.get(`/social/search?q=${encodeURIComponent(q)}`, { withCredentials: true });
  ...
  const sortedStories = useMemo(() => {
    return [...otherStories].sort((a, b) => {
      const aHasUnviewed = a.stories?.some(s => !s.viewedBy?.includes(myUserId));
      const bHasUnviewed = b.stories?.some(s => !s.viewedBy?.includes(myUserId));
  ...
  return (
    <div className="w-full min-h-[100dvh] overflow-x-hidden pb-20 lg:pb-0 font-sans antialiased relative">
      ...
      <div className="max-w-7xl mx-auto pt-2 sm:pt-3 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_330px] gap-6 xl:gap-8">
          ...
          <div className="w-full max-w-2xl mx-auto space-y-5">
            ...
            <div className="relative group/storybar">
              <button aria-label="Scroll left" onClick={() => scrollStories(-1)} className="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 w-8 h-8 bg-white dark:bg-slate-800 rounded-full shadow-md z-10 hidden lg:group-hover/storybar:flex items-center justify-center text-slate-500 hover:text-slate-800 transition-colors">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button aria-label="Scroll right" onClick={() => scrollStories(1)} className="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 w-8 h-8 bg-white dark:bg-slate-800 rounded-full shadow-md z-10 hidden lg:group-hover/storybar:flex items-center justify-center text-slate-500 hover:text-slate-800 transition-colors">
              ...
              <div ref={storyContainerRef} className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-white/40 dark:border-slate-700/50 rounded-3xl py-3 px-4 shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-none flex items-center gap-3 overflow-x-auto scrollbar-none snap-x snap-mandatory select-none transition-colors duration-300 scroll-smooth">
                ...
                <div className="flex flex-col shrink-0">
                  {myStoryGroup ? (
                    <div
                      onClick={() => { setActiveStoryGroup(myStoryGroup); setActiveStoryIndex(0); }}
                    ...
                    >
                      {myStoryGroup.stories[0]?.mediaType === 'video' ? (
                        <video src={myStoryGroup.stories[0]?.media} className="w-full h-full object-cover" />
                      ) : (
                        <img loading="lazy"
                          src={myStoryGroup.stories[0]?.media || myStoryGroup.stories[0]?.mediaUrl || myStoryGroup.stories[0]?.image || getAvatarUrl(user?.pic, user?.img, user?.name)}
                          alt="Your story"
                      ...
                      <div
                        onClick={(e) => { e.stopPropagation(); setShowStoryModal(true); }}
                        className="absolute bottom-2 left-2 right-2 flex justify-between items-end"
                      >
                        <span className="text-[10px] font-bold text-white truncate drop-shadow-md">My Story</span>
                    ...
                    >
                      ...
                      <div className="absolute bottom-3 left-0 right-0 flex flex-col items-center justify-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-white drop-shadow-md">My Story</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {loadingStories
                  ...
                  : sortedStories.map((group) => {
                      const hasUnviewed = group.stories?.some(s => !s.viewedBy?.includes(myUserId));
                      return (
                        <div
                          key={group.userId}
                          ...
                          className="w-[84px] h-[120px] rounded-2xl overflow-hidden relative cursor-pointer hover:scale-105 transition-all duration-300 shadow-sm group shrink-0"
                        >
                        {(group.stories[0]?.media || group.stories[0]?.mediaUrl || group.stories[0]?.image) ? (
                          group.stories[0]?.mediaType === 'video' ? (
                            <video src={group.stories[0]?.media} className="w-full h-full object-cover" />
                          ) : (
                            <img loading="lazy"
                              src={group.stories[0].media || group.stories[0].mediaUrl || group.stories[0].image}
                              alt={group.userName}
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                e.target.onerror = null;
                        ...
                        <div className={`absolute top-2 left-2 w-8 h-8 rounded-full p-[2px] ${hasUnviewed ? 'bg-gradient-to-tr from-pink-500 to-purple-500' : 'bg-slate-300'}`}>
                          <img loading="lazy"
                            src={getAvatarUrl(group.userPic, group.userImg, group.userName)}
                            alt={group.userName}
                            className="w-full h-full rounded-full object-cover border-2 border-white"
                        ...
                        {onlineUsersMap[group.userId] && (
                          <div className="absolute top-2 right-2 w-3 h-3 bg-emerald-500 rounded-full border-2 border-white shadow-sm" title="Online" />
                        )}
            ...
            {loadingMemories ? (
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl p-8 sm:p-16 text-center shadow-[0_10px_30px_rgba(15,23,42,0.05)] dark:shadow-none min-h-[300px] flex flex-col items-center justify-center transition-colors duration-300">
                ...
                <h3 className="text-xl font-black text-slate-800 dark:text-white">Oops, something went wrong!</h3>
              ...
              <div className="bg-white dark:bg-slate-800 border border-[#8b5cf614] dark:border-slate-700 rounded-3xl p-8 sm:p-16 text-center shadow-[0_10px_30px_rgba(15,23,42,0.05)] dark:shadow-none min-h-[420px] flex flex-col items-center justify-center transition-colors duration-300">
                ...
                <h3 className="text-xl font-black text-[#111827] dark:text-white transition-colors duration-300">No posts yet</h3>
              ...
              <div className="space-y-6">
                {memories.map((post) => {
                  const hasLiked = post.likes?.some(id => (id?._id || id)?.toString() === myUserId);
                      ...
                      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-700/50">
                          ...
                          <div className="min-w-0 flex flex-col justify-center">
                            <div className="flex items-center gap-2">
                              ...
                              {(post.userId?.isVerified || post.isVerified) && (
                                <span className="bg-blue-500 text-white p-[2px] rounded-full flex items-center justify-center shrink-0" title="Verified Traveler">
                        ...
                        <div className="ml-auto relative group/menu flex items-center justify-center shrink-0">
                          ...
                          <div className="absolute right-0 top-full mt-1 opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all duration-200 z-50 bg-white dark:bg-slate-800 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-slate-100 dark:border-slate-700 py-1.5 min-w-[140px]">
                            {!isCreator && (
                              <button
                                onClick={() => setReportModal({ isOpen: true, targetId: post._id, targetType: "post", reportedUserId: post.userId?._id || post.userId })}
                              ...
                              <>
                                {isCreator && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      e.currentTarget.closest('.group\\/menu').querySelector('.absolute').classList.remove('opacity-100', 'visible');
                      ...
                      >
                        ...
                        {post.tags?.length > 0 && (
                          <div className="absolute top-3 left-3 flex flex-wrap gap-1 z-10 pointer-events-none">
                            {post.tags.slice(0, 3).map((tag, i) => (
                              <span key={i} className="bg-black/50 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-auto">
                                #{tag}
                              </span>
                            ))}
                          </div>
                        ...
                        {post.music && post.music.title && (
                          ...
                          >
                            ...
                            {post.music.preview && (
                              <button
                                onClick={(e) => { e.stopPropagation(); toggleAudio(post._id); }}
                      ...
                      <div className="px-4 pt-3 pb-1">
                        <div className="flex items-center justify-between w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl px-4 py-2 rounded-[20px] border border-white/80 dark:border-slate-700 shadow-sm transition-colors duration-300">
                          <div className="flex items-center gap-6 shrink-0 overflow-hidden">
                            ...
                            >
                              <span className={`text-[15px] transition-all duration-300 ${hasLiked ? "drop-shadow-[0_0_8px_rgba(250,204,21,0.7)] scale-110" : "opacity-75 group-hover:drop-shadow-[0_0_4px_rgba(250,204,21,0.4)] group-hover:scale-110 group-hover:opacity-100"} leading-none`}>
                                ✨
                              </span>
                              <span className={`text-[12px] font-bold transition-colors duration-300 ${hasLiked ? "bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent" : "text-slate-600 group-hover:text-amber-500"}`}>
                          ...
                          >
                            <Bookmark className={`w-5 h-5 ${isSaved ? "text-[#6C4DF6] fill-[#6C4DF6]" : "text-slate-500 group-hover:text-[#6C4DF6]"} transition-colors`} />
                          </button>
                        </div>
                        ...
                        {(activeCommentPost === post._id ? visibleComments : previewComments).length > 0 && (
                          <>
                            {/* Divider */}
                            ...
                            <div className="mt-3 space-y-2 pl-1">
                              {(activeCommentPost === post._id ? visibleComments : previewComments).map((comment) => (
                                <div key={comment._id} className="text-[14px] group relative pr-6 leading-tight">
                                  <span className="font-semibold text-slate-800 mr-1.5">{comment.userName}</span>
                        ...
                        {visibleCommentsCount > previewComments.length && activeCommentPost !== post._id && !commentsLoadingMap[post._id] && (
                          <button
                            onClick={() => handleOpenComments(post._id)}
                ...
                {hasMore && memories.length > 0 && (
                  <div className="flex justify-center mt-6 pb-6" ref={(el) => {
                    if (el) {
                    ...
                    {loadingMore && <Loader2 className="w-6 h-6 animate-spin text-[#6C4DF6]" />}
                  </div>
                )}
              </div>
            )}
      ...
      <AnimatePresence>
        {showViewersList && activeStoryGroup && activeStoryGroup.userId?.toString() === myUserId && (
          <>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              onClick={() => { setShowViewersList(false); setIsStoryPaused(false); }}
            />
            <motion.div
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <div>
                              <p className="text-[14px] font-bold text-slate-800 dark:text-white group-hover:text-[#6C4DF6] transition-colors">{v.userId?.name}</p>
                          ...
                          {reaction && (
                            <div className="text-2xl animate-bounce drop-shadow-sm">{reaction.emoji}</div>
                          )}
                        </div>
                      );
                    })}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700">
                <h3 className="text-lg font-black text-slate-900 dark:text-white">Edit Post</h3>
                <button
                  onClick={() => { setShowEditPostModal(false); setEditPostData(null); }}
              ...
              <form onSubmit={handleEditPostSubmit} className="p-5 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider pl-1">Caption</label>
                  <textarea
      ...
      {reportModal.isOpen && (
        <ReportModal
          ...
          onClose={() => setReportModal({ isOpen: false, targetId: null, targetType: "post", reportedUserId: null })}
