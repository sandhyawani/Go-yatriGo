...
import { Lock, ArrowRight, Eye, EyeOff, ShieldCheck, AlertCircle, Fingerprint, Sparkles, ChevronLeft } from "lucide-react";
...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`/auth/reset-password/${token || otp}`, { password });
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden relative">
      <div className="absolute inset-0 z-0 bg-cover bg-center opacity-10 scale-105" style={{ backgroundImage: `url(${travelBg})` }} />
        ...
        <div className="hidden lg:flex lg:w-3/5 items-center justify-center p-12">
          ...
          >
            ...
            <div className="relative bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-2 overflow-hidden shadow-2xl">
              <img src={stickerPack} alt="Travel Hero" className="w-full h-[80vh] object-cover rounded-[2rem] transform group-hover:scale-[1.02] transition-transform duration-700 opacity-80" />
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 bg-black/20">
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white">Secure Recovery</span>
                </motion.div>
                <motion.h2
                ...
                >
                  Reset <br /><span className="text-brand-400">Password.</span>
                </motion.h2>
              </div>
            </div>
          </motion.div>
        ...
        <div className="w-full lg:w-2/5 flex flex-col items-center justify-center p-6 sm:p-12 relative overflow-y-auto custom-scrollbar">
          ...
          >
            <div className="mb-5 flex flex-col items-center lg:items-start">
              ...
              <h1 className="text-2xl font-black text-slate-900 tracking-tighter">New Password</h1>
              <p className="text-slate-500 font-medium text-[11px] mt-1">Choose a strong new password for your account.</p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit} noValidate>
              ...
              {!token && (
                <div className="space-y-1">
                  <label htmlFor="otp" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                    OTP
                  </label>
                  <div className="relative group">
                    <Fingerprint className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.otp ? "text-red-500" : "text-slate-400 group-focus-within:text-brand-500"}`} />
                    <motion.input
                  ...
                  <AnimatePresence>
                    {errors.otp && (
                      <motion.div id="otp-error" role="alert" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-1.5 mt-1 ml-1 text-red-500">
                        <AlertCircle className="w-3 h-3 shrink-0" />
                        <span className="text-[10px] font-bold">{errors.otp}</span>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
              ...
              {/* New Password */}
              <div className="space-y-1">
                <label htmlFor="new-password" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                  New Password
                </label>
                <div className="relative group">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword((p) => !p)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors p-1">
                ...
                <AnimatePresence>
                  {errors.password && (
                    <motion.div id="pw-error" role="alert" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-1.5 mt-1 ml-1 text-red-500">
                      <AlertCircle className="w-3 h-3 shrink-0" />
                ...
                {password && !errors.password && (
                  <div id="pw-strength" className="mt-2 ml-1" aria-live="polite">
                    <div className="flex gap-1 h-1 w-full rounded-full overflow-hidden bg-slate-100">
                      <div className={`h-full transition-all duration-300 ${meta.bar}`} style={{ width: `${strength}%` }} />
                    </div>
                    <span className={`text-[10px] font-bold mt-1 block ${meta.textColor}`}>
                      Strength: {meta.text}
                    </span>
                  </div>
                )}
              ...
              {/* Confirm Password */}
              <div className="space-y-1">
                <label htmlFor="confirm-password" className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                  Confirm Password
                </label>
                <div className="relative group">
                  <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${errors.confirm ? "text-red-500" : "text-slate-400 group-focus-within:text-brand-500"}`} />
                  <motion.input
                    whileFocus={{ scale: 1.01 }}
                    id="confirm-password"
                  ...
                  />
                  <button type="button" onClick={() => setShowConfirmPassword((p) => !p)} aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors p-1">
                ...
                <AnimatePresence>
                  {errors.confirm && (
                    <motion.div id="confirm-error" role="alert" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-1.5 mt-1 ml-1 text-red-500">
                      <AlertCircle className="w-3 h-3 shrink-0" />
                      <span className="text-[10px] font-bold">{errors.confirm}</span>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              >
                ...
                {loading ? (
                  <span className="text-xs uppercase tracking-widest">Updating…</span>
                ) : (
                  <>
                    <span className="uppercase tracking-widest text-xs relative z-10">Update Password</span>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex flex-col items-center">
              <Link to="/login" className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-600 font-black text-[9px] uppercase tracking-[0.4em] transition-colors group">
