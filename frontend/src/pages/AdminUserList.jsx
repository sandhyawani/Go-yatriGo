...
const AdminUserList = ({ columns }) => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: statsLoading
        ? null
        : stats?.reports ?? stats?.metrics?.reportsPending ?? 0,
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(20).setTextColor(2, 132, 199);
    doc.setFont("helvetica", "bold");
    doc.text("GoYatriGo", 14, 15);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10).setTextColor(50, 55, 65);
    doc.text("User Directory Report", 14, 23);
    doc.text(`Report Generated: ${dateStr}`, 14, 30);
    doc.text("Go YatriGo | Travel Community Platform", 14, 37);
    doc.autoTable({
      head: [tableColumn],
      body: tableRows,
      styles: { fontSize: 7 },
    ...
    doc.save(`User-Directory-Report_${dateStr}.pdf`);
  };
  ...
  return (
    <div className="user-management-page relative min-h-[calc(100vh-72px)] overflow-hidden bg-white px-3 pb-6 pt-4 sm:px-4 lg:px-6">
      ...
      <div className="relative mx-auto max-w-[1450px] space-y-4">
        ...
        >
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-brand shadow-[0_0_10px_rgba(2,132,199,0.8)]" />
              <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-brand">
                Trust and Safety
              </span>
            </div>
            ...
            <p className="mt-1 text-xs text-text-secondary">
              Monitor accounts, permissions, and moderation actions in one workspace.
            </p>
            {error && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-brand-200 bg-white/55 px-3.5 text-xs font-medium text-text-primary shadow-sm backdrop-blur-xl transition-all duration-200 hover:border-brand-300 hover:bg-brand-50 hover:text-brand-dark disabled:cursor-not-allowed disabled:opacity-50"
            >
        ...
        >
          {metrics.map(
            ({ label, value, detail, icon: Icon, tone, filterValue }, index) => (
              <motion.article
                key={label}
                custom={index}
                ...
                <span
                  className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${
                    activeFilter === filterValue
                      ...
                      : "bg-brand-100 text-brand-dark group-hover:bg-brand-200"
                  }`}
                >
                  <Icon className="h-5 w-5" />
                </span>
                <div className="relative min-w-0 flex-1">
                  <p className="text-lg font-bold tracking-tight text-text-primary tabular-nums leading-tight">
                    {value === null ? (
        ...
        <Datatable
          columns={columns}
          onDirectoryChange={refreshDirectoryMetrics}
