...
const VerificationModal = ({ isOpen, onClose, profileUserId, user, profileUser, updateUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setUploadProgress(0);
    try {
      const uploadedGovIdUrl = await uploadImageToCloudinary(govIdFile, setUploadProgress);
      ...
      showToast.success("Verification Submitted", "Your document is pending review.");
      handleClose();
    } catch (err) {
      console.error(err);
  ...
  const inputClass = "w-full rounded-xl border border-[#E5E7EB] bg-white px-5 py-3 text-sm font-semibold text-[#1E293B] outline-none transition-all duration-200 ease-out placeholder:text-[#64748B]/50 focus:border-[#7C3AED] focus:ring-4 focus:ring-[#7C3AED]/10";
    ...
    className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
      ...
      className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <h3 className="text-lg font-black text-slate-800">Verify Identity</h3>
          <button type="button" onClick={handleClose} disabled={isSubmitting} className="p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
          <div>
            ...
            <div className="relative">
              <button
                ...
                className={`w-full flex items-center justify-between text-left px-4 py-3 bg-white border ${isDropdownOpen ? 'border-brand-500 ring-2 ring-brand-500/20' : 'border-slate-200'} rounded-xl text-slate-900 font-bold outline-none transition-all text-sm shadow-sm ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:border-brand-300'}`}
              >
                <span className={govIdType ? "text-slate-900" : "text-slate-400"}>
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  ...
                  >
                    {["Aadhaar Card", "PAN Card", "Passport", "Driving License"].map((type) => (
                      <button
                        key={type}
                        type="button"
                        onClick={() => {
                        ...
                        className={`w-full text-left px-4 py-3 text-sm font-semibold transition-colors hover:bg-brand-50 ${govIdType === type ? 'bg-brand-50 text-brand-600' : 'text-slate-700'}`}
                      >
                        {type}
                      </button>
                    ))}
          ...
          <div>
            ...
            <div className="relative group">
              <ShieldCheck className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors text-slate-400`} />
              <label
              htmlFor="modalGovIdFile"
              className={`w-full pl-11 pr-4 py-3 bg-white border border-slate-200 focus:border-brand-500 hover:border-brand-300 rounded-xl text-slate-900 font-bold outline-none transition-all text-sm shadow-sm flex items-center justify-between cursor-pointer ${!govIdType || isSubmitting ? "opacity-50 pointer-events-none" : ""}`}>

                <span className="truncate">
                  {govIdFile ? govIdFile.name : "Choose File..."}
                </span>
                {govIdPreview &&
                <img src={govIdPreview} alt="Preview" className="h-6 w-10 object-cover rounded shadow-sm border border-slate-200 ml-3 shrink-0" />}
          ...
          <div className="mt-2">
              <div className="h-1 w-full rounded-full bg-slate-100 overflow-hidden">
                <div className="h-full rounded-full bg-[#7C3AED] transition-all duration-200" style={{ width: `${uploadProgress}%` }} />
              </div>
              <p className="mt-1 text-[9px] font-bold text-slate-400">Uploading... {uploadProgress}%</p>
            </div>}
          ...
          <div className="mt-4 flex gap-3">
            ...
            className="flex-1 py-3 rounded-xl text-sm font-semibold text-white bg-[#7C3AED] hover:bg-[#6D28D9] transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
...
const Profileupdate = () => {
  ...
  const [preferredTravelStyle, setPreferredTravelStyle] = useState(profileUser?.preferredTravelStyle || "");
  const [favoriteDestinations, setFavoriteDestinations] = useState(profileUser?.favoriteDestinations || []);
  ...
  [name, username, city, stateVal, mobile, bio, interests, preferredTravelStyle, favoriteDestinations]
  );
  ...
  const inputClass =
  "w-full rounded-xl border border-[#E5E7EB] bg-white px-5 py-3 text-sm font-semibold text-[#1E293B] outline-none transition-all duration-200 ease-out placeholder:text-[#64748B]/50 focus:border-[#7C3AED] focus:ring-4 focus:ring-[#7C3AED]/10";

  const labelClass =
  "mb-2 ml-2 block select-none text-[10px] font-bold uppercase tracking-wider text-[#64748B]";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (file) {
        const uploadedImageUrl = await uploadImageToCloudinary(file, setUploadProgress);
  ...
  return (
    <div className="relative min-h-screen bg-[#FDFDFD] px-4 pb-24 md:pb-8 pt-0 font-sans antialiased text-[#1E293B] sm:px-6 lg:px-8">
      <div className="pointer-events-none absolute top-10 right-0 h-[30rem] w-[30rem] rounded-full bg-gradient-to-br from-[#7C3AED]/10 to-fuchsia-400/10 blur-[80px]" />
      <div className="pointer-events-none absolute bottom-40 left-0 h-[20rem] w-[20rem] rounded-full bg-gradient-to-tr from-[#7C3AED]/10 to-transparent blur-[80px]" />
      ...
      <div className="relative z-10 mx-auto max-w-5xl space-y-2">
        ...
        noValidate>
          ...
          className="lg:col-span-5 flex flex-col gap-3">
            ...
            {}
            <div className="rounded-3xl border border-[#E5E7EB]/60 bg-white p-6 text-center shadow-soft relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-[#7C3AED]/5 to-transparent"></div>

              <div className="group relative inline-block select-none mb-4 mt-2">
                <div className="relative mx-auto h-32 w-32 overflow-hidden rounded-full border-[6px] border-white shadow-xl ring-4 ring-[#7C3AED]/20 transition-transform duration-300 group-hover:scale-105">
                  <img
                  src={
                  preview ||
                  getAvatarUrl(
                  ...
                  <label
                  htmlFor="file"
                  className="absolute inset-0 flex cursor-pointer flex-col items-center justify-center bg-[#7C3AED]/80 text-white opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100">
                ...
                {profileUser?.isVerified === true && profileUser?.verificationStatus === "verified" &&
                <div className="absolute bottom-1 right-1 bg-[#7C3AED] rounded-full p-1.5 border-2 border-white shadow-md">
              ...
              <h3 className="text-xl font-black text-[#1E293B] tracking-tight truncate px-2">
                {name || "Explorer"}
              </h3>
            ...
            {}
            <div className="rounded-3xl border border-[#E5E7EB]/60 bg-white p-6 shadow-soft relative overflow-hidden">
              <div>
                <label htmlFor="bio" className={labelClass}>
                  Bio / About Me
                </label>
                <div className="group relative flex-1">
                  <AlignLeft className="absolute left-4 top-4 h-4 w-4 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                  <textarea
                  id="bio"
                  rows="3"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-1">
                  ...
                  <div
                  key={idx}
                  className="flex items-center gap-2 px-2 py-1 bg-white border border-[#7C3AED]/20 text-[#7C3AED] text-[10px] font-bold rounded-xl shadow-sm">
                    ...
                    className="text-[#7C3AED]/60 hover:text-[#7C3AED] transition-colors focus:outline-none">
                ...
                <div className="flex gap-2">
                  ...
                  className="px-4 py-3 bg-[#7C3AED]/10 text-[#7C3AED] rounded-2xl text-[10px] font-black uppercase tracking-wider hover:bg-[#7C3AED]/20 transition-colors">
              ...
              <div className="mt-4">
                ...
                <div className="group relative">
                  <Compass className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                  <CustomSelect
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  ...
                  className="flex items-center gap-2 px-2.5 py-1 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-bold rounded-xl shadow-sm">
                    ...
                    type="button"
                    onClick={() =>
                    setFavoriteDestinations(favoriteDestinations.filter((_, i) => i !== idx))}
                ...
                <div className="flex gap-2">
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      {
                        setFavoriteDestinations([...favoriteDestinations, newFavDest.trim()]);
                        setNewFavDest("");
                      }
                    }
                  }} />
                  ...
                  className="px-4 py-3 bg-emerald-50 text-emerald-700 rounded-2xl text-[10px] font-black uppercase tracking-wider hover:bg-emerald-100 transition-colors border border-emerald-200">
            ...
            <div className="rounded-3xl border border-[#E5E7EB]/60 bg-white p-6 shadow-soft relative overflow-hidden">
              <div className="mb-5 flex items-center justify-between border-b border-slate-100 pb-4 select-none">
                <div className="flex items-center gap-4">
                  <div className="rounded-2xl border border-[#7C3AED]/20 bg-gradient-to-br from-[#7C3AED]/10 to-fuchsia-500/10 p-3 text-[#7C3AED]">
                    <UserIcon className="h-5 w-5" />
                  </div>
                  <div>
                    <h2 className="text-xl font-black tracking-tight text-[#1E293B]">
                      {pageMeta.title}
                    </h2>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                  <div>
                    ...
                    <div className="group relative">
                      <User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                      <input
                      id="name"
                  ...
                  <div>
                    ...
                    <div className="group relative">
                      <AtSign className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                      <input
                      id="username"
                      className={`${inputClass} pl-11`}
                      value={username}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="group relative">
                      <Phone className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                      <input
                      id="mobile"
                      className={`${inputClass} pl-11`}
                      value={mobile}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mt-4">
                  <div>
                    ...
                    <div className="group relative">
                      <MapPin className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-[#7C3AED]" />
                      <CustomSelect
                      value={stateVal}
                  ...
                  <div>
                    ...
                    <div className="group relative">
                      ...
                      disabled={!stateVal}
                      searchable={true}
                      options={stateVal ? INDIAN_STATES_AND_CITIES[stateVal].map(c => ({ value: c, label: c })) : []}
                ...
                <div className="mt-8 border-t border-slate-100 pt-6 mb-4">
                  <h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wider">Traveler Verification</h3>
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-emerald-700">Your traveler identity has been verified.</p>
                      </div>
                    </div> :
                  ...
                  <div className="bg-red-50 border border-red-100 rounded-2xl p-5 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-red-700 mb-3">{profileUser?.verificationNote || "Please upload a clear, valid Government ID."}</p>
                        <button type="button" onClick={() => setIsVerificationModalOpen(true)} className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-800 text-xs font-bold rounded-xl transition-colors">
                          Try Again
                        </button>
                      </div>
                    </div> :
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mb-3">Verify your identity to build trust with other travelers.</p>
                        <button type="button" onClick={() => setIsVerificationModalOpen(true)} className="px-4 py-2 bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-xs font-bold rounded-xl transition-colors">
                          Verify Identity
                        </button>
                      </div>
                    </div>}
                ...
                <div className="pt-1">
                  ...
                  className="flex h-11 w-full md:w-auto md:px-8 items-center justify-center gap-2 rounded-xl text-sm font-semibold text-white bg-[#7C3AED] hover:bg-[#6D28D9] shadow-soft transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-70">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-20 flex h-[4.5rem] items-center justify-around border-t border-slate-100 bg-white/90 backdrop-blur-md px-4 pb-safe md:hidden shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
          <Link
        to="/"
        className="flex flex-col items-center gap-1 text-slate-400 hover:text-[#7C3AED] transition-colors p-2">
        ...
        className="relative -top-5 flex flex-col items-center">
            ...
            <div className="bg-[#7C3AED] text-white p-3.5 rounded-2xl shadow-lg shadow-[#7C3AED]/30 transform rotate-3 hover:rotate-6 transition-transform">
        ...
        className="flex flex-col items-center gap-1 text-slate-400 hover:text-[#7C3AED] transition-colors p-2">
          ...
          <Link
        to="/profile"
        className="flex flex-col items-center gap-1 text-[#7C3AED] transition-colors p-2">

            <div className="relative">
              <UserIcon className="h-5 w-5" />
              <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-[#7C3AED] rounded-full"></div>
            </div>
