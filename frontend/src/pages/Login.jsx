...
const Login = () => {
  ...
  const { user, loading, login, loginWithGoogle } =
  ...
  useEffect(() => {
    if (user) {
      navigate("/social/buddy", { replace: true });
    }
  }, [user, navigate]);
  ...
  useEffect(() => {
    ...
    if (!clientId || clientId.includes("test-google-client-id")) {
      console.error(
        "Google Sign-In: REACT_APP_GOOGLE_CLIENT_ID is missing or invalid."
      );
      return;
    }
  ...
  const handleGoogleLogin = useCallback(() => {
    ...
    if (
      !clientId ||
      clientId.includes("test-google-client-id")
    ) {
      Swal.fire({
        icon: "info",
        title: "Google Sign-In",
        text:
          "Google authentication is not configured yet. Please configure REACT_APP_GOOGLE_CLIENT_ID in .env.",
        confirmButtonColor: "#0284c7",
        customClass: {
          popup: "rounded-[1.5rem]",
        },
    ...
    if (!window.google?.accounts?.id) {
      Swal.fire({
        ...
        text:
          "Google Sign-In SDK is still loading or could not be reached. Please check your connection and try again.",
        confirmButtonColor: "#0284c7",
        customClass: {
          popup: "rounded-[1.5rem]",
        },
      ...
      try {
        const justRegistered =
          sessionStorage.getItem(
            "goyatrigo_just_registered"
          ) === "true" ||
          localStorage.getItem(
            `goyatrigo_newly_registered_${userEmail}`
          ) === "true";

        if (justRegistered && myId) {
          localStorage.setItem(
            `goyatrigo_is_new_user_${myId}`,
            "true"
          );
  ...
  return (
    <div className="min-h-[calc(100vh-40px)] bg-slate-50 flex font-sans overflow-hidden relative">
      ...
      <div className="relative z-20 w-full flex flex-col lg:flex-row min-h-[calc(100vh-40px)]">
        {/* Left Travel Image */}
        <div className="hidden lg:flex lg:w-3/5 items-center justify-center p-6">
          <motion.div
            initial={{
              opacity: 0,
              scale: 0.9,
          ...
          >
            ...
            <div className="relative bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-2 overflow-hidden shadow-2xl">
              <img
                src={stickerPack}
                alt="Travel Hero"
                className="w-full max-h-[68vh] object-cover rounded-[2rem] transform group-hover:scale-[1.02] transition-transform duration-700"
              />
            </div>
          </motion.div>
        </div>
        ...
        {/* Right Login Form */}
        <div className="w-full lg:w-2/5 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-6 relative custom-scrollbar">
          <motion.div
            initial={{
              opacity: 0,
              x: 20,
            ...
            className="w-full max-w-[420px] bg-white border border-slate-100 p-5 sm:p-7 rounded-[1.75rem] sm:rounded-[2rem] shadow-xl my-2 lg:my-0"
          >
