...
import { X, Search, MoreHorizontal, Eye, UserMinus, AlertTriangle, RefreshCw, Users, UserCheck } from "lucide-react";
...
export const RelationsModal = ({
  ...
  const otherName = profileUser?.name || profileUser?.username || "this traveler";

  // Determine modal title and subtitle
  let title = "Network";
  let subtitle = "Manage connections";
  if (relationsModalType === "followers") {
    title = "Followers";
    subtitle = isOwnProfile ? "People who follow you" : `People who follow ${otherName}`;
  } else if (relationsModalType === "following") {
    title = "Following";
    subtitle = isOwnProfile ? "People you follow" : `People ${otherName} follows`;
  } else if (relationsModalType === "trip_mates") {
    title = "Trip Mates";
    subtitle = isOwnProfile ? "Your travel connections" : `Travel companions of ${otherName}`;
  } else if (relationsModalType === "mutuals") {
    title = "Mutuals";
    subtitle = isOwnProfile ? "People who follow each other" : `Mutual connections of ${otherName}`;
  }
  ...
  // Push internal search to parent
  useEffect(() => {
  ...
  const handleConfirmUnfollow = async () => {
    ...
    if (handleFollowToggleForUser) {
      ...
      showToast.success(`Updated relationship with ${user.name}`);
    }
  ...
  const handleRetry = () => {
    if (openRelationsModal) {
      openRelationsModal(relationsModalType);
    }
  };
                    ...
                    <p className="text-sm font-semibold text-slate-600 text-center">
                      {relationsModalType === "mutuals"
                        ? isOwnProfile
                          ...
                          : `${otherName} has no mutual connections yet.`
                        : relationsModalType === "following"
                        ? isOwnProfile
                          ? "You are not following anyone yet."
                          : `${otherName} is not following anyone yet.`
                        : isOwnProfile
                        ? "You have no followers yet."
                        : `${otherName} has no followers yet.`}
                    </p>
                    <p className="text-xs text-slate-500 mt-1 text-center">
                      {relationsModalType === "mutuals"
                        ? "When you and another traveler follow each other, they'll appear here."
                        : "Explore and connect!"}
                    </p>
                  </div>
                )}
