...
const notifIcon = (type) => {
  if (type === "post_like") return <Heart className="w-3.5 h-3.5 text-[#FF5A7A] fill-[#FF5A7A]" />;
  if (type === "post_comment") return <MessageSquareIcon className="w-3.5 h-3.5 text-[#6C4DF6]" />;
  if (type === "follow" || type === "new_follower" || type === "follow_request") return <UserPlus className="w-3.5 h-3.5 text-emerald-500" />;
  if (type === "story_reply") return <Sparkles className="w-3.5 h-3.5 text-amber-500" />;
  if (type === "message_request") return <MessageSquare className="w-3.5 h-3.5 text-blue-500" />;
  if (type === "join_request") return <Compass className="w-3.5 h-3.5 text-purple-500" />;
  return <Bell className="w-3.5 h-3.5 text-slate-500" />;
};
...
const SocialSidebar = () => {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        const res = await axios.get(`/social/search?q=${encodeURIComponent(searchQuery)}`, { withCredentials: true });
  ...
  const handleAcceptMessage = async (e, roomId, notificationId) => {
    e.stopPropagation();
    try {
      await axios.put(`/chat/direct/${roomId}/accept`, {}, { withCredentials: true });
  ...
  const handleRejectMessage = async (e, roomId, notificationId) => {
    e.stopPropagation();
    try {
      await axios.put(`/chat/direct/${roomId}/decline`, {}, { withCredentials: true });
      setNotifications(prev => prev.filter(n => n._id !== notificationId));
    } catch(err) { console.error(err); }
  };
  ...
  const handleAcceptJoin = async (e, groupId, requestId, notificationId) => {
    e.stopPropagation();
    try {
      await axios.post(`/social/buddy/manage-request/${groupId}`, { requestId, status: "Approved" }, { withCredentials: true });
      setNotifications(prev => prev.filter(n => n._id !== notificationId));
    } catch(err) { console.error(err); }
  };
  ...
  const handleRejectJoin = async (e, groupId, requestId, notificationId) => {
    e.stopPropagation();
    try {
      await axios.post(`/social/buddy/manage-request/${groupId}`, { requestId, status: "Rejected" }, { withCredentials: true });
      setNotifications(prev => prev.filter(n => n._id !== notificationId));
    } catch(err) { console.error(err); }
  };

  const handleNotificationClick = async (n) => {
    if (n.type === "follow" || n.type === "new_follower" || n.type === "follow_accept" || n.type === "follow_request") {
      if (n.sender?._id) navigate(`/profile/${n.sender._id}`);
    } else if (n.type === "group_joined" || n.type === "trip_cancelled" || n.type === "group" || n.type === "join_request") {
      const groupId = typeof n.group === 'object' ? n.group?._id : n.group;
      if (groupId) navigate(`/social/buddy/${groupId}`);
    } else if (n.type === "new_message" || n.type === "message_request" || n.type === "direct") {
    ...
    if (!n.isRead) {
      try {
        await axios.put(`/notifications/${n._id}/read`, {}, { withCredentials: true });
    ...
    <>
            ...
            <AnimatePresence>
              {showNotifPanel && (
                ...
                >
                  ...
                  <div className="overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-purple-200 hover:scrollbar-thumb-purple-300 scrollbar-track-transparent">
                    {notifications.length === 0 ? (
                      <div className="py-16 text-center flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm text-slate-500 font-bold">No new travel updates ✈️</p>
                      </div>
                    ) : (
                      <div className="flex flex-col">
                        ...
                        {[
                          ...
                          { title: "Yesterday", items: groupedNotifs.yesterday },
                          ...
                          <div key={group.title}>
                            <div className="sticky top-0 bg-white/95 backdrop-blur-sm z-10 px-5 py-2 border-y border-slate-50 mt-1 first:mt-0">
                              <span className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest">{group.title}</span>
                            </div>
                            <div className="flex flex-col">
                                ...
                                {group.items.map((n) => (
                                  <motion.div
                                    ...
                                    className={`relative flex items-start gap-4 px-5 py-4 transition-all duration-200 cursor-pointer group hover:bg-purple-50 border-b border-slate-50 last:border-b-0 ${
                                      n.isRead ? "bg-white" : "bg-purple-50/40"
                                    }`}
                                  >
                                    <div className="relative shrink-0">
                                      <img
                                        src={getAvatarUrl(n.sender?.pic, n.sender?.img, n.sender?.name)}
                                        alt={n.sender?.name}
                                    ...
                                    <div className="flex-1 min-w-0 pt-0.5">
                                      <p className={`text-[13px] leading-relaxed text-slate-700 ${n.isRead ? "font-normal" : "font-semibold text-slate-900"}`}>
                                        <span className="font-extrabold text-slate-900 mr-1">{n.sender?.name}</span>
                                      ...
                                      {n.type === "follow_request" && (
                                        <div className="flex gap-2 mt-3">
                                          <button onClick={(e) => handleAcceptRequest(e, n.sender?._id)} className="px-4 py-1.5 bg-[#6C4DF6] text-white text-[11px] font-bold rounded-lg hover:bg-[#5b3ee0] hover:shadow-md transition-all active:scale-95">Accept</button>
                                          <button onClick={(e) => handleRejectRequest(e, n.sender?._id)} className="px-4 py-1.5 bg-slate-100 text-slate-600 text-[11px] font-bold rounded-lg hover:bg-slate-200 transition-colors active:scale-95">Reject</button>
                                        </div>
                                      )}
                                      {n.type === "message_request" && n.room && (
                                        <div className="flex gap-2 mt-3">
                                          <button onClick={(e) => handleAcceptMessage(e, n.room, n._id)} className="px-4 py-1.5 bg-[#6C4DF6] text-white text-[11px] font-bold rounded-lg hover:bg-[#5b3ee0] hover:shadow-md transition-all active:scale-95">Accept</button>
                                          <button onClick={(e) => handleRejectMessage(e, n.room, n._id)} className="px-4 py-1.5 bg-slate-100 text-slate-600 text-[11px] font-bold rounded-lg hover:bg-slate-200 transition-colors active:scale-95">Reject</button>
                                        </div>
                                      )}
                                      {n.type === "join_request" && n.joinRequest && (
                                        <div className="flex gap-2 mt-3">
                                          <button onClick={(e) => handleAcceptJoin(e, typeof n.group === 'object' ? n.group._id : n.group, n.joinRequest, n._id)} className="px-4 py-1.5 bg-[#6C4DF6] text-white text-[11px] font-bold rounded-lg hover:bg-[#5b3ee0] hover:shadow-md transition-all active:scale-95">Approve</button>
                                          <button onClick={(e) => handleRejectJoin(e, typeof n.group === 'object' ? n.group._id : n.group, n.joinRequest, n._id)} className="px-4 py-1.5 bg-slate-100 text-slate-600 text-[11px] font-bold rounded-lg hover:bg-slate-200 transition-colors active:scale-95">Decline</button>
                                        </div>
                                      )}
                                    </div>
                                    {!n.isRead && (
                      ...
                      <div className="flex items-center gap-3 px-3 py-3 rounded-xl opacity-60 cursor-not-allowed">
                        ...
                        <div className="flex-1">
                          <p className="text-[13px] font-extrabold text-slate-800 flex items-center justify-between">
                            Go Live
                            <span className="px-2 py-0.5 bg-purple-100 text-purple-600 text-[9px] font-bold uppercase rounded-full tracking-wider">Soon</span>
                          </p>
                          <p className="text-[10px] text-slate-500 font-medium">Broadcast to followers</p>
                        </div>
                      </div>
                    )}
                  </Menu.Item>
        ...
        <div className="border-t border-slate-100 mt-2 pt-2">
          {user ? (
            ...
            <div className="flex flex-col gap-2">
              <Link to="/login" className="btn-secondary py-2 px-4 text-center text-sm font-semibold">
                Sign In
              </Link>
              <Link to="/register" className="btn-primary py-2 px-4 text-center text-sm font-semibold">
                Sign Up
              </Link>
            </div>
          )}
      ...
      <div className="lg:hidden sticky top-0 bg-white/95 backdrop-blur-md z-[990] px-4 h-12 border-b border-slate-100 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-[17px] font-black tracking-tight text-slate-900">Go YatriGo.</span>
        </Link>
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      {user && (
        <nav className="lg:hidden fixed bottom-4 left-4 right-4 bg-white/80 backdrop-blur-xl border border-white/50 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.08)] z-[990] h-[60px] flex justify-around items-center px-2">
          <Link
            to="/"
            className={`p-2.5 rounded-xl transition-all ${
              location.pathname === "/" ? "bg-[#6C4DF6]/10 text-[#6C4DF6]" : "text-slate-400"
            }`}
          >
          ...
          <button
            onClick={() => setIsSearchOpen(true)}
            className={`p-2.5 rounded-xl transition-all ${
              isSearchOpen ? "bg-[#6C4DF6]/10 text-[#6C4DF6]" : "text-slate-400"
            }`}
          >
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition-all relative ${
              showNotifPanel ? "bg-[#6C4DF6]/10 text-[#6C4DF6]" : "text-slate-400"
            }`}
          >
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#FF5A7A] text-white text-[8px] font-black rounded-full flex items-center justify-center">
                {unreadCount > 9 ? "9+" : unreadCount}
              </span>
            )}
          </button>
          ...
          <Link
            to="/profile"
            className={`p-2.5 rounded-xl transition-all ${
              location.pathname === "/profile" ? "bg-[#6C4DF6]/10 text-[#6C4DF6]" : "text-slate-400"
            }`}
          >
          ...
          >
              <div className="p-5">
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => { setIsOpen(false); setIsCreatePostOpen(true); }}
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-800">Post a Memory</p>
                      <p className="text-[11px] text-slate-500">Share your travel photos</p>
                    </div>
                  </button>
                  <Link
                    to="/social/buddy/new"
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-800">Create a Trip</p>
                      <p className="text-[11px] text-slate-500">Host a travel squad</p>
                    </div>
                  </Link>
                </div>
              </div>
                ...
                {searchLoading ? (
                  ...
                  <div className="space-y-5">
                    {(searchTab === "all" || searchTab === "travelers") && (
                      <div className="space-y-2">
                        {searchTab === "all" && searchResults.travelers?.length > 0 && (
                        ...
                        {searchResults.travelers?.length > 0 ? (
                          searchResults.travelers.map((traveler) => (
                            <div
                              key={traveler._id}
                              onClick={() => { setIsSearchOpen(false); navigate(`/profile/${traveler._id}`); }}
                            ...
                            >
                              <img
                                src={traveler.pic || `https://ui-avatars.com/api/?name=${encodeURIComponent(traveler.name)}&background=6C4DF6&color=fff&bold=true`}
                                alt={traveler.name}
                                className="w-9 h-9 rounded-full object-cover border-2 border-white shadow-sm shrink-0"
                                onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(traveler.name)}&background=6C4DF6&color=fff&bold=true`; }}
                              />
                          ...
                          <p className="text-xs text-slate-400 text-center py-4">No travelers found</p>
                        ) : null}
                      </div>
                    )}
                    ...
                    {(searchTab === "all" || searchTab === "groups") && (
                      <div className="space-y-2">
                        {searchTab === "all" && searchResults.trips?.length > 0 && (
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Active Trips</span>
                        )}
                        {searchResults.trips?.length > 0 ? (
                          searchResults.trips.map((trip) => (
                            <div
                              key={trip._id}
                              onClick={() => { setIsSearchOpen(false); navigate(`/social/buddy/${trip._id}`); }}
                              className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-white border border-transparent hover:border-[#6C4DF6]/15 hover:shadow-md hover:scale-[1.01] transition-all cursor-pointer group"
                            >
                    ...
                    {(searchTab === "all" || searchTab === "posts") && (
                      <div className="space-y-2">
                        {searchTab === "all" && searchResults.memories?.length > 0 && (
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Memories</span>
                        )}
                        {searchResults.memories?.length > 0 ? (
                          ...
                          <p className="text-xs text-slate-400 text-center py-4">No posts found</p>
                        ) : null}
                      </div>
                    )}

                    {searchTab === "all" && !searchResults.travelers?.length && !searchResults.trips?.length && !searchResults.memories?.length && (
                      <div className="text-center py-14 flex flex-col items-center gap-2">
                        <div className="w-10 h-10 rounded-full bg-rose-50 flex items-center justify-center">
                          <Search className="w-5 h-5 text-rose-300" />
                        </div>
                        <p className="text-xs font-bold text-rose-400 uppercase tracking-wider">No results</p>
