...
const MemberSelector = ({
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="grid grid-cols-2 gap-2 sm:gap-3 bg-background p-1.5 rounded-2xl">
        {tabs.map((t) => {
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white pl-11 pr-4 py-3 rounded-2xl border border-slate-200 text-xs text-text-primary placeholder-slate-400 focus:outline-hidden focus:border-brand shadow-xs"
        />
      </div>
      ...
      <div className="max-h-72 overflow-y-auto pr-1 space-y-2.5 custom-scrollbar">
        {loading ? (
          <div className="p-8 text-center space-y-2">
            ...
            <p className="text-xs text-text-muted">Scanning traveler network...</p>
          </div>
        ) : error ? (
          <div className="p-8 text-center bg-red-50 rounded-3xl border border-dashed border-red-200">
            <ShieldCheck className="w-8 h-8 text-red-400 mx-auto mb-2 opacity-60" />
            <p className="text-xs font-bold text-text-primary">
              Couldn't load travelers.
            </p>
            <button onClick={fetchTravelers} className="mt-3 text-brand text-xs font-bold hover:underline">
              Retry
            </button>
          </div>
                ...
                <div
                  ...
                  className={`p-3.5 rounded-2xl border transition-all cursor-pointer flex items-center justify-between group select-none ${
                    isSelected
                      ...
                      : "bg-white border-slate-200 hover:border-brand/40 shadow-xs"
                  }`}
                >
                    ...
                    <div className="flex-1 min-w-0">
                          ...
                          <span
                            ...
                            className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border ${
                              badge === "Trip Mate"
                                ...
                                : "bg-background text-text-secondary border-slate-200"
                            }`}
                          >
                            {badge}
                          </span>
                      ...
                      {Boolean(u.tripsCount && u.tripsCount > 0) && (
                        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-50 text-text-secondary text-[10px] font-semibold border border-slate-200">
                            {u.tripsCount} {u.tripsCount === 1 ? "Journey Together" : "Journeys Together"}
                          </span>
                          {lastTripName && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 text-[10px] font-semibold border border-amber-100 truncate max-w-[160px]">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shrink-0 ${
                      isSelected
                        ...
                        : "bg-background text-text-primary hover:bg-brand/10 hover:text-brand"
                    }`}
                  >
            ...
            {filteredList.length > displayLimit && (
              <div className="pt-2 pb-1 text-center">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-background hover text-xs font-bold text-text-primary transition-all border border-slate-200/60"
                >
