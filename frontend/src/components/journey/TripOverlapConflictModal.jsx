 ...
 * 
 * Displays a high-visibility, responsive modal popup when a user encounters a
 * trip overlap conflict (ACTIVE_JOURNEY_CONFLICT or OVERLAPPING_JOURNEY).
 * 
 * Renders with z-[100] and fixed positioning above all page elements,
 * ensuring no clipping on mobile, tablet, or desktop.
 */
const TripOverlapConflictModal = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full w-[calc(100%-1.5rem)] sm:w-full max-w-md max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-amber-200 dark:border-amber-900/50 p-5 sm:p-6 flex flex-col items-center text-center my-auto transition-all animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500"
        ...
        {/* Warning Icon Badge */}
        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800/60 rounded-2xl sm:rounded-3xl flex items-center justify-center mb-3.5 sm:mb-4 shadow-sm shrink-0">
          <AlertTriangle className="w-7 h-7 sm:w-8 sm:h-8 text-amber-500 dark:text-amber-400" />
        </div>

        {/* Title */}
        <h2
          id="overlap-conflict-title"
          className="text-lg sm:text-xl font-black text-slate-900 dark:text-white mb-2 leading-snug px-2"
        >
          {title}
        </h2>

        {/* Primary Message */}
        <div className="bg-amber-50/90 dark:bg-amber-950/40 border border-amber-200/90 dark:border-amber-800/60 rounded-2xl p-3.5 sm:p-4 mb-4 w-full text-left">
          <p className="text-xs sm:text-sm font-bold text-amber-950 dark:text-amber-200 m-0 leading-relaxed">
            {primaryMessage}
          </p>
        </div>

        {/* Details / Explanation */}
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed px-1">
          {isActiveConflict
        ...
        {conflictingTrip && (
          <div className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 rounded-2xl p-3.5 mb-5 text-left space-y-1.5 text-xs">
