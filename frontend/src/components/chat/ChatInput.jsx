...
export const ChatInput = ({
  ...
  if (isJourneyCancelled) {
    return (
      <div className="p-3.5 bg-amber-50 border-t border-amber-200 text-amber-800 text-xs font-semibold text-center flex items-center justify-center gap-2 shrink-0">
  ...
  return (
    <div className="p-2 sm:p-3 bg-white border-t border-slate-100 shrink-0 relative flex flex-col">
      ...
      className="absolute -top-12 right-6 bg-brand text-white shadow-soft hover:bg-brand-dark transition-all z-20 flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-full hover:scale-105 active:scale-95 duration-200 animate-in fade-in">
      ...
      <div className="mb-2 p-2.5 bg-slate-50 border-l-4 border-brand rounded-r-xl flex items-center justify-between shadow-soft mx-1 sm:mx-2 mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
        ...
        className="p-1 text-text-muted hover:text-text-secondary hover:bg-background rounded-full transition-colors">
      ...
      <div className="mb-2 self-start flex items-center gap-2 bg-background px-3 py-1.5 rounded-full mx-1 sm:mx-2 text-[12px] font-medium text-text-primary">
          ...
          <button onClick={() => setSelectedFile(null)} className="ml-1 text-text-muted hover:text-rose-500">
      ...
      <div className="mb-2 self-start flex items-center gap-3 bg-rose-50 px-4 py-2 rounded-full mx-1 sm:mx-2 border border-rose-100 shadow-sm animate-pulse w-full max-w-sm justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={cancelVoiceRecording} className="p-1.5 bg-slate-200 text-text-secondary rounded-full hover">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>}
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 bg-white rounded-xl h-[48px] px-1.5 sm:px-2.5 shadow-soft border border-border-default focus-within:border-brand focus-within:ring-4 focus-within:ring-brand/10 transition-all duration-200 mx-1 sm:mx-2 mb-1">
        ...
        className="hidden sm:flex w-8 h-8 sm:w-9 sm:h-9 rounded-full items-center justify-center shrink-0 bg-transparent hover:bg-brand-50 text-text-muted hover:text-brand transition-colors duration-200">
        ...
        className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center shrink-0 transition-colors duration-200 bg-transparent ${isSending ? "text-slate-300 pointer-events-none" : "hover:bg-brand-50 text-text-muted hover:text-brand"}`}>
        ...
        "bg-brand hover:bg-brand-dark text-white shadow-soft hover:scale-105 active:scale-95" :
        "bg-background text-slate-300 cursor-not-allowed shadow-none"
        }`}>
