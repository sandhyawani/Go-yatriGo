...
export const ChatInput = ({
  ...
  return (
    <div className="p-3 bg-white border-t border-slate-100 shrink-0 relative flex flex-col">
      ...
      {}
      {replyToMsg &&
      <div className="mb-2 p-2.5 bg-slate-50 border-l-4 border-[#7C3AED] rounded-r-xl flex items-center justify-between shadow-soft mx-2 mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      {showEmojiPicker &&
      <div className="absolute bottom-full left-4 mb-2 z-50 shadow-soft rounded-xl overflow-hidden border border-[#E5E7EB]/60 bg-white min-h-[350px] w-[320px] flex items-center justify-center">
      ...
      <div className="flex items-center gap-1.5 bg-white rounded-xl h-[48px] px-2.5 shadow-soft border border-[#E5E7EB] focus-within:border-[#7C3AED] focus-within:ring-4 focus-within:ring-[#7C3AED]/10 transition-all duration-200 mx-2 mb-1">
        <button
        onClick={() => setShowEmojiPicker((prev) => !prev)}
        type="button"
        aria-label="Open emoji picker"
        className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 bg-transparent hover:bg-[#F3E8FF] text-[#64748B] hover:text-[#7C3AED] transition-colors duration-200">
        ...
        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 transition-colors duration-200 bg-transparent ${isSending ? "text-slate-300 pointer-events-none" : "hover:bg-[#F3E8FF] text-[#64748B] hover:text-[#7C3AED]"}`}>
        ...
        rows={1}
        className="flex-1 max-h-[80px] bg-transparent text-sm text-slate-800 px-2 py-1.5 resize-none outline-none focus:outline-none focus:ring-0 focus:border-transparent focus-visible:outline-none focus-visible:ring-0 border-none shadow-none placeholder-slate-400 overflow-y-auto scrollbar-none min-h-[32px]" />
