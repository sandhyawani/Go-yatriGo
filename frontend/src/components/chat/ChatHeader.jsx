...
import { ArrowLeft, Users, Phone, Video, MoreVertical, Trash2, Compass } from "lucide-react";
...
export const ChatHeader = ({
  ...
  const other = activeRoom.type === "direct"
    ? activeRoom.members?.find((member) => (member._id || member)?.toString() !== currentUserId?.toString())
    : null;
  const otherId = other?._id || other;
  const isOnline = otherId && onlineUsers.has(otherId);
  const isBlocked = otherId && user?.blockedUsers?.includes(otherId);
    ...
    <div className="h-16 px-5 bg-white/90 backdrop-blur-md border-b border-slate-100 flex justify-between items-center shrink-0 z-50 sticky top-0 shadow-[0_2px_10px_-5px_rgba(0,0,0,0.02)]">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          >
            {activeRoom.type === "direct" && isBlocked ? (
      ...
      <div className="flex items-center gap-1">
        ...
        <div ref={headerOptionsRef} className="relative">
          ...
          {showHeaderOptions && (
            <div className="absolute right-0 top-full mt-1.5 z-[9999] w-56 bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.15)] border border-slate-100 overflow-hidden py-1 flex flex-col">
