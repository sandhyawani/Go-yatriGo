...
const MemoryCard = ({ item, user }) => {
  ...
  const getBadgeInfo = (postType) => {
    switch (postType) {
      case "story":
        return { icon: <Clapperboard className="w-3 h-3" />, label: "Dispatch", bg: "bg-white/90 text-brand-600" };
      case "group":
  ...
  const handleCardClick = () => {
    if (item.type === "group") navigate(`/social/buddy/${item._id}`);else
    if (item.type === "story") navigate(`/social`);else
