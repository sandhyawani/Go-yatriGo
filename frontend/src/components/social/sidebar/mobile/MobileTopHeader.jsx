  ...
  return (
    <div className="lg:hidden sticky top-0 bg-white z-[990] px-3 sm:px-4 h-12 border-b border-slate-100 flex justify-between items-center">
      {location.pathname.startsWith("/settings/") ? (
        <Link
          to="/settings"
          className="flex items-center gap-1.5 sm:gap-2 text-slate-800 font-semibold text-[14px] sm:text-[15px] hover:text-brand-600 transition-colors truncate"
        >
          <ICONS.ArrowLeft className="w-5 h-5 text-brand-600 shrink-0" />
          <span className="truncate">Settings</span>
        </Link>
      ) : (
        <Link to="/social/buddy" className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          <div className="relative w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-brand-600 flex items-center justify-center shadow-md shadow-brand-600/10 overflow-hidden shrink-0">
            <span className="relative z-10 text-white font-bold text-xs tracking-tighter">
              GY
            </span>
          </div>
          <span className="text-[15px] sm:text-[17px] font-bold tracking-tight text-slate-900 truncate font-heading">
            Go YatriGo
          </span>
        </Link>
      )}
        ...
        {user && (
          <>
            ...
            <button
              ...
              className="bell-btn relative p-1.5 sm:p-2 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors"
              aria-label="Notifications"
            >
              <ICONS.Bell className="w-4.5 h-4.5" />
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 min-w-[15px] h-3.5 px-0.5 text-[9px] font-extrabold flex items-center justify-center bg-rose-500 text-white rounded-full ring-2 ring-white animate-pulse">
