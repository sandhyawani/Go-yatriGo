...
const CompactMemoryCard = ({ item }) => {
  ...
  const getBadgeInfo = (postType) => {
    switch (postType) {
      ...
      case "group":
        return { icon: <Users className="w-3 h-3" />, label: "Travel Group", bg: "bg-white/90 text-blue-600" };
      case "document":
  ...
  const isNonTravel = item.postType === "document" || item.postType === "profile_update";
  ...
  return (
    <motion.div
      ...
      `}
    >
      <div className={`relative w-full h-full overflow-hidden ${isNonTravel ? 'p-4' : ''}`}>
        {mediaList.length > 0 ? (
          <>
          ...
          <div className={`w-full h-full flex flex-col items-center justify-center p-4 text-center ${isNonTravel ? 'bg-white rounded-xl shadow-sm' : 'bg-slate-100'}`}>
            <div className={`p-4 rounded-full mb-3 ${isNonTravel ? 'bg-slate-50' : 'bg-white shadow-sm'}`}>
              {React.cloneElement(badge.icon, { className: 'w-6 h-6 text-slate-400' })}
            </div>
            <span className="text-sm font-medium text-slate-600 line-clamp-2">{item.caption || item.title || "No Preview"}</span>
          </div>
        )}

        {/* Top Badge */}
        <div className={`absolute top-3 left-3 z-10 ${isNonTravel ? 'top-5 left-5' : ''}`}>
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full backdrop-blur-md text-[10px] font-bold tracking-wide shadow-sm ${badge.bg}`}>
            {badge.icon}
            {badge.label}
          </div>
        </div>
        ...
        <div className={`absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 ${isNonTravel ? 'm-4 rounded-xl' : ''}`}>
