...
const StorySticker = ({
  ...
  if (isEdit) {
    return (
      ...
      dragElastic={0}
      dragMomentum={false}
      className="absolute pointer-events-auto cursor-grab active:cursor-grabbing z-50"
      style={{
      ...
      onWheel={(e) => {
        ...
        onUpdate(sticker.id, { scale: Math.max(0.5, Math.min(currentScale + scaleChange, 3)) });
      }}>
  ...
  return (
    ...
    style={{
      ...
      <div className="-translate-x-1/2 -translate-y-1/2 inline-flex flex-col items-center justify-center">
        {content}
      </div>
    </motion.div>);
