...
const CreateDispatchModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSelectAndClose = (track) => {
    ...
    setStickers((prev) => {
      const existingMusic = prev.find((s) => s.type === "music");
      const newStickers = prev.filter((s) => s.type !== "music");
      newStickers.push({
        id: Date.now().toString() + Math.random(),
        type: "music",
        xPercent: existingMusic?.xPercent ?? 50,
        yPercent: existingMusic?.yPercent ?? 50,
        scale: existingMusic?.scale ?? 1,
        data: songData
      });
      return newStickers;
    });
