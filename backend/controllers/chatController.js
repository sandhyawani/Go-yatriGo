...
exports.getOrCreateDirectRoom = async (req, res) => {
  try {
    ...
    if (!room) {
      ...
      const whoCanMsg = targetUser.privacySettings?.whoCanMessage || "everyone";
      if (whoCanMsg === "none") {
...
exports.getUserRooms = async (req, res) => {
  try {
    ...
    {
      $unwind: { path: "$journeyId", preserveNullAndEmptyArrays: true }
    },
    {
      $lookup: {
        from: "messages",
...
exports.deleteChatForMe = async (req, res) => {
  try {
    ...
    if (room.type === "group" && room.travelGroupId) {
      ...
      return res.status(200).json({ success: true, message: "Chat hidden successfully" });
    } else {
      await ChatRoom.findByIdAndUpdate(roomId, {
        $addToSet: { hiddenFor: userId }
      });
